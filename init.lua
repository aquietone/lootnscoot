...
function LNS.GetItemFromDB(itemName, itemID, rules, exact)
    ...
    local rowsFetched = db.GetItemFromDB(itemName, itemID, query, LNS)
...
function LNS.addToItemDB(item)
    ...
    db.AddItemToDB(LNS, itemID, itemName, value, itemIcon)
end
...
function LNS.findItem(itemName, itemId, exact, maxResults)
    ...
    local counter, retTable = db.FindItemInDB(query, param, maxResults, LNS)
...
function LNS.lookupLootRule(mq_item, itemID, tablename, item_link)
    ...
    if tablename == nil then
        -- check global rules
        local found = false
        found, rule, classes, lookupLink = db.CheckRulesDB(itemID, db.PreparedStatements.CHECK_DB_PERSONAL, LNS)
        which_table = 'Personal'
        if not found then
            found, rule, classes, lookupLink = db.CheckRulesDB(itemID, db.PreparedStatements.CHECK_DB_GLOBAL, LNS)
            which_table = 'Global'
        end
        if not found then
            found, rule, classes, lookupLink = db.CheckRulesDB(itemID, db.PreparedStatements.CHECK_DB_NORMAL, LNS)
            which_table = 'Normal'
        end
...
function LNS.MainLoop()
    while not LNS.Terminate do
        ...
        if settings.TempSettings.DoImport then
            ...
            db.ImportOldRulesDB(settings.TempSettings.ImportDBFilePath, LNS)
        ...
        if settings.TempSettings.doBulkSet then
            ...
            db.BulkSet(settings.TempSettings.BulkSet, settings.TempSettings.BulkRule,
                settings.TempSettings.BulkClasses, settings.TempSettings.BulkSetTable, doDelete, LNS)
